	 ...
	 uint32_t _ulFreq, uint32_t _ulDutyCycle)
{
	...
	/* 如果这里因为占空比的万分比是0 或 10000而没有配置TIM的通道 */
	/* 将会导致后面无法再修改PWM的占空比 */
	/* 这里选择注销掉 */
	// if (_ulDutyCycle == 0)
	// {		
...
/*
*函数说明：	对TIMx通道设置新的PWM占空比
*说明补充：	在使用本函数前，先用bsp_SetTIMOutPWM()配置好TIMx的配置
*输入形参：	TIMx:定时器结构体
*			_ucChannel：PWM通道
*			_ulDutyCycle：占空比（0-10000，单位：万分比）
*输出数据：	无
*/
void bsp_TIMx_Set_NewDutyCycle(TIM_TypeDef* TIMx, uint8_t _ucChannel, uint32_t _ulDutyCycle)
{
	switch(_ucChannel)
	{
		case TIM_CHANNEL_1:
			TIMx->CCR1 = (TIMx->ARR*_ulDutyCycle)/10000;
			break;
		
		case TIM_CHANNEL_2:
			TIMx->CCR2 = (TIMx->ARR*_ulDutyCycle)/10000;
			break;

		case TIM_CHANNEL_3:
			TIMx->CCR3 = (TIMx->ARR*_ulDutyCycle)/10000;
			break;

		case TIM_CHANNEL_4:
			TIMx->CCR4 = (TIMx->ARR*_ulDutyCycle)/10000;
			break;
...
*输出数据：	无
*/
void bsp_TIMx_Set_NewFrequency(TIM_TypeDef* TIMx,uint32_t _ulFreq)
{
	TIM_HandleTypeDef TIMx_Frequency = {0};
	uint16_t usPeriod;
	uint16_t usPrescaler;
	uint32_t uiTIMxCLK;

	__IO uint32_t DutyCycle_specific_value[4] = {0};		/* 原占空比的比值（万分比） */


	/* 失能TIMx */
	TIMx_Frequency.Instance = TIMx;
	__HAL_TIM_DISABLE(&TIMx_Frequency);

	/* 计算并保存原占空比 */
	DutyCycle_specific_value[0] = ((TIMx->CCR1*10000)/TIMx->ARR);
	DutyCycle_specific_value[1] = ((TIMx->CCR2*10000)/TIMx->ARR);
	DutyCycle_specific_value[2] = ((TIMx->CCR3*10000)/TIMx->ARR);
	DutyCycle_specific_value[3] = ((TIMx->CCR4*10000)/TIMx->ARR);
	...
	TIMx->PSC = usPrescaler;
	TIMx->ARR = usPeriod;

	TIMx->CCR1 = (usPeriod*DutyCycle_specific_value[0])/10000;
	TIMx->CCR2 = (usPeriod*DutyCycle_specific_value[1])/10000;
	TIMx->CCR3 = (usPeriod*DutyCycle_specific_value[2])/10000;
	TIMx->CCR4 = (usPeriod*DutyCycle_specific_value[3])/10000;

	/* 使能TIMx */
	__HAL_TIM_ENABLE(&TIMx_Frequency);
}
